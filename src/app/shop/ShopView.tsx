...
export default async function ShopView({
  ...
  const sort =
    (searchParams.sort as ProductFilters['sort']) ?? 'newest';

  const categorySlug =
    categoryObj?.slug ?? searchParams.category;

  const collectionSlug =
    collectionObj?.slug ?? searchParams.collection;
  ...
  if (collectionSlug) {
    baseParams.collection = collectionSlug;
  }

  if (searchParams.min_price) {
    baseParams.min_price = searchParams.min_price;
  }

  if (searchParams.max_price) {
    baseParams.max_price = searchParams.max_price;
  }

  if (searchParams.size) {
    baseParams.size = searchParams.size;
  }
